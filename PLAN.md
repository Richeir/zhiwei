...
>
> 文档状态：v2 · 数据通道修订为微博 Web 接口（路线 B：隐藏 WebView 代理）· 待评审
> 修订记录：v2 — 放弃开放平台申请路线，改用 Web 端接口 + WebView 会话代理；D9、R1、R2、R7、M1 等章节随之调整
...
| D6 | 状态与数据 | Zustand（UI 状态）+ TanStack Query（服务端数据）+ `WebViewChannel` 页面内 fetch（替代 Axios，见 D9） | 轻量、纯 JS、无原生依赖，天然四平台兼容 |
| D7 | UI 组件 | 自建薄组件层 + React Native Paper（可选主题层） | 避免依赖只支持手机端的组件库；桌面差异用扩展文件处理 |
| D8 | 语言 | TypeScript 全覆盖 | — |
| D9 | **数据通道（路线 B）** | App 常驻**离屏 WebView**（保持已登录的 weibo.com 同源会话），所有 API 调用经 `injectJavaScript` 在**页面内发起 `fetch`**；`src/api/` 保留后端抽象（`web/` 默认实现，`openapi/` 留位） | 免申请、免审核，接口能力即网页全量能力；Cookie（含 httpOnly 的 `SUB`）由原生 CookieJar 管理，无需四端各写 Cookie 桥，`X-XSRF-TOKEN` 等参数页面自带；代价 = 风控长期维护（R1）+ WebView 常驻内存 |
...
| **R2** | Web 会话维护：扫码登录、Cookie 过期、多端登录互踢 | 中 | 登录统一在嵌入式 WebView 人工完成（App 不碰账密/加密参数）；请求命中 401/跳登录页即自动唤起重新登录；会话持久化依赖系统 CookieJar，四端分别验证 |
...
| **R7** | `react-native-webview` 在 macOS/Windows 的支持与"离屏常驻 + 页面内 fetch"模式的稳定性未实测 | 高（路线 B 地基） | M0 首个 spike 四端验证：隐藏 WebView 加载 weibo.com → 注入 fetch → 取回 JSON；失败则桌面端回退"原生 Cookie 桥 + 直连 HTTP"方案，或收缩桌面端范围 |
...
- [ ] `react-native-webview` 离屏代理 spike 四端通过（R7，M0 首项）
...
- [ ] 存储层封装：KV（`@react-native-async-storage/async-storage`，四端可用），仅存偏好/草稿/搜索历史；**登录凭证一律不落 App 存储**（Cookie 只存在于系统 WebView CookieJar）
...
- [ ] 退出登录：登出 + 清理 WebView 会话/Cookie（macOS/Windows 按平台 API 分别处理）
- [ ] 重启后会话恢复验证（依赖系统 CookieJar 持久化，四端各自实测）
- [ ] DoD：四端均可扫码登录 → 重启保持会话 → 登出后需重新登录
...
| react-native-webview | ✅ | ✅ | ⚠️ | ✅ | **路线 B 核心依赖**（登录 + API 通道）；macOS/Windows 稳定性为 M0 首要 spike（R7） |
|（新库按需追加）| | | | | |
...
- [ ] Mock：以 `WebViewChannel` 为注入边界做 fake（回放 Web 端点契约 JSON），四端共用，UI 测试不依赖真微博
...
- [ ] 登录凭证仅存于系统 WebView CookieJar；App 自有存储与日志中无任何 Cookie/凭证（四端验证）
...
| M1 | 登录与会话（WebViewChannel 基座） | 1 周 | 四端扫码登录 + 通道基座稳定 |
...
| **合计** | | **约 14–17 周（并行后 ≈ 13 周）** | |

> 关键路径：M0 的 **WebViewChannel 四端 spike（R7）** 与 RN 版本对齐 spike（R3）——开放平台权限不再是阻塞项，但 R7 是路线 B 的地基，失败即触发方案评审（回退原生 Cookie 桥或收缩桌面端）。任一 spike 失败应在 M0 结束时定案，不带病进入 M1。
